...
// Note: If there is more than one colour which is odd in number, then the first colour in the array which is present odd number of times 
// is displayed. The colours of the balloons can all be either upper case or lower case in the array. 
...
public class p11                                                   
{
    public static void main(String gg[])
    {
        ...
        for(int i=0;i<n;i++)
        {
            freq[Character.toLowerCase(arr[i])-'a']++;
        }

        boolean found=false;
        for(int i=0;i<n;i++)
        {
            if(freq[Character.toLowerCase(arr[i])-'a']%2!=0)
            {
